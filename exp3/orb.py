...
def calc_homography(match_pts):
    ...
    # eigen_M = np.linalg.eig(temp_M)[1][-1].reshape(3,3)
...
# 为图像添加径向畸变, 避免透视作用下可视化效果不美观
def CalcUndistort(img, mtx, dist):
    return cv2.undistort(img, mtx, dist*40, dst=None, newCameraMatrix=None)
...
if __name__ == '__main__':
    ...
    for i in range(1,5):
        print(i)
        img1 = cv2.imread('datas/'+str(i+1)+'.png')
